my.preforecast.fun <- function(dtList) {
  ...
  for (i in 1:length(dtList)) {
    ...
    train_end_date <- c(2019, 2)
    test_start_date <- c(2019, 3)
    test_end_date <- c(2020, 2)
    ...
    tsModelList <- list(
      ...
      nnet.model = nnetar(rides_ts_train, P = 12),
    ...
    accuracy_results <- data.frame(
      ...
      RMSE.train = c(tsModelList$ets.accuracy[3], tsModelList$arima.accuracy[3], tsModelList$stlets.accuracy[3], tsModelList$stlarima.accuracy[3], tsModelList$tbats.accuracy[3], tsModelList$nnet.accuracy[3], tsModelList$hybrid.accuracy[3]),
      RMSE.test = c(tsModelList$ets.accuracy[4], tsModelList$arima.accuracy[4], tsModelList$stlets.accuracy[4], tsModelList$stlarima.accuracy[4], tsModelList$tbats.accuracy[4], tsModelList$nnet.accuracy[4], tsModelList$hybrid.accuracy[4]),
      MAPE.train = c(tsModelList$ets.accuracy[9], tsModelList$arima.accuracy[9], tsModelList$stlets.accuracy[9], tsModelList$stlarima.accuracy[9], tsModelList$tbats.accuracy[9], tsModelList$nnet.accuracy[9], tsModelList$hybrid.accuracy[9]),
      ...
      MASE.train = c(tsModelList$ets.accuracy[11], tsModelList$arima.accuracy[11], tsModelList$stlets.accuracy[11], tsModelList$stlarima.accuracy[11], tsModelList$tbats.accuracy[11], tsModelList$nnet.accuracy[11], tsModelList$hybrid.accuracy[11]),
  ...
  # Save the accuracy results to my files (AJUSTADO A TU RUTA)
  base_dir <- "/home/carlos/IA/FINAL/time-series_forecasting"
  dir.create(file.path(base_dir, "data"), recursive = TRUE, showWarnings = FALSE)

  write_xlsx(
    accuracy_df,
    path = file.path(base_dir, "data", "accuracy_results_fullForecastDec.xlsx")
  )

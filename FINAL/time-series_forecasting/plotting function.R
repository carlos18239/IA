my.plot.fun <- function(forecastList){
  # Esta función toma la lista de modelos (tsList_full o tsList_pre)
  # y genera una figura con los 7 modelos por agencia.
  
  # === Ajusta tu ruta base aquí ===
  base_dir <- "/home/carlos/IA/FINAL/time-series_forecasting"
  output_dir <- file.path(base_dir, "plots", "full_series_Dec")
  ...
  for (i in 1:length(forecastList)) {
    ...
    plot_ets <- autoplot(fets, main = "ETS", series = "Forecast", 
                         xlab = "Date", ylab = "Ridership (UPT)") +
    ...
    plot_tbats <- autoplot(ftbats, main = "TBATS", xlab = "Date", 
                           ylab = "Ridership (UPT)") +
    ...
    output_filename <- file.path(
      output_dir,
      paste0("forecast_plot_", names(forecastList[i]), ".png")
    )
